...
update = function() {
    try {
        var templatePath = path.resolve(files.getRootPath(), 'template');
        ...
        Logs.info('clone git ...');
        clone("https://github.com/particles4dev-team/sample-project.git", templatePath, { ignoreCertErrors: 1 }).then(function (repo) {
            Logs.info('done !');
        }, function (err) {
            throw err;
        })
