...
function createProject (name){
    ncp(path.resolve(files.getRootPath(), 'template', __templateFolder, 'project'), path.resolve(files.getCurentPath(), name), function (err) {
        if (err) {
...
function createModule (name){
    ncp(path.resolve(files.getRootPath(), 'template', __templateFolder, 'package'), path.resolve(files.getCurentPath(), name), function (err) {
        if (err) {
            return Logs.error(err);
        }
        Logs.info('done !');
